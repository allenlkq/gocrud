...
func printAndGetUser(uid string) (user User) {
	result, err := store.NewQuery(uid).UptoDepth(1).Run()
	if err != nil {
		log.Fatalf("Error: %v", err)
	}
...
func main() {
	...
	store.Get().Init("localhost", "crudtest", "instructions")

	search.Get().Init("memsearch")
	*/
/*
	indexer.Register("User", SimpleIndexer{})
	indexer.Register("Role", SimpleIndexer{})
	indexer.Register("Device", SimpleIndexer{})
	indexer.Register("Tool", SimpleIndexer{})
	*//*
	...
	err := store.NewUpdate("User", uid).SetSource(uid).AddChild("Role").
		Set("name", "admin").Execute(c)
	if err != nil {
		log.Fatalf("Error: %v", err)
	}

	err = store.NewUpdate("User", uid).SetSource(uid).AddChild("Role").
		Set("name", "teacher").Execute(c)
	if err != nil {
		log.Fatalf("Error: %v", err)
	}
	printAndGetUser(uid)
